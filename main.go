...
type App struct {
	...
	termIn     io.WriteCloser // writes go to terminal display
	termOut    io.ReadCloser  // SSH output feeds into terminal
...
func (a *App) buildUI() {
	...
	sidePanel := container.NewVBox(
		...
		widget.NewSeparator(),
		uploadBtn,
		widget.NewLabel("Arraste arquivos para a janela"),
		widget.NewSeparator(),
		widget.NewLabel("Diretório remoto:"),
		a.pwdLabel,
	)

	// fyne-terminal: emulador xterm completo, lida com tudo
	a.term = fyneterm.New()
...
func (a *App) connect() {
	...
	go func() {
		...
		// Pipe: terminal → SSH stdin
		termReader, termWriter := io.Pipe()
		// Pipe: SSH stdout → terminal display
		...
		// Inject PROMPT_COMMAND via a separate channel to track pwd
		// We use a side-channel session so it doesn't pollute the terminal pipe
		go a.trackPwd(client)

		// Connect fyne-terminal to the SSH pipes
		// RunWithConnection feeds SSH output into the terminal emulator
		// and forwards terminal keypresses to SSH stdin
		go func() {
...
// trackPwd polls the remote pwd every 2s via a separate SSH session.
...
func (a *App) uploadFile(path string) {
	...
	go func() {
		...
		// SCP protocol: send header then data then null byte
		// Remote scp overwrites existing files automatically
